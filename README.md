#  Dashboard for Monitoring Coolant Status using React, Firebase and ESP32

<p align="center">

![React](https://img.shields.io/badge/React-Frontend-blue?logo=react)
![Firebase](https://img.shields.io/badge/Firebase-Realtime%20Database-orange?logo=firebase)
![ESP32](https://img.shields.io/badge/ESP32-IoT-red)

</p>

##  Live Dashboard

🔗 **Dashboard Link:**
https://coolantdashm.netlify.app/

> **Note:** The dashboard works only when the ESP32 is powered on, as it displays real-time data received from the device.

##  Dashboard Screenshots

<img width="1918" height="1079" alt="Screenshot 2025-07-26 144049" src="https://github.com/user-attachments/assets/ef4a996c-599e-4ba0-9852-ddc4a2ef1b88" />

<br><br>

<img width="1918" height="1078" alt="Screenshot 2025-07-26 144254" src="https://github.com/user-attachments/assets/e4ca1995-77d2-4a03-83cf-f64f41a88096" />
